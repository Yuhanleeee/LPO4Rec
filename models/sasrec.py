...
class LayerNorm(nn.Module):
    ...
    def __init__(self, features, eps=1e-6):  ## raw eps=1e-6
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        # Compute RMS (root mean square) along the last dimension
...
class SublayerConnection(nn.Module):
    ...
    def __init__(self, hidden_size, dropout):
        ...
        # self.norm = RMSNorm(hidden_size)  ## bad than LayerNorm
...
class SASRec(nn.Module):
    def __init__(self, args):
        ...
        self.pred_layer = Linear_pred(args, args.item_nums+1)
    
    def forward(self, hidden, masks):
        for transformer_temp in self.Trans_seq_rep:
            hidden = transformer_temp.forward(hidden, masks)
        prod = self.pred_layer(hidden[:,-1,:])  ## Linear Prediction
        return prod, hidden[:,-1,:]
